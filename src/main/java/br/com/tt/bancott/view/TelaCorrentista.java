...
public class TelaCorrentista implements Tela {
	...
	private void cadastrarCorrentista() {
		...
		if (tipoPessoa == TipoPessoa.PF) {
			tipoDocumento = this.pedirTipoDocumento();
		...
		if (tipoPessoa == TipoPessoa.PF) {
			correntista = new CorrentistaPF(nomeCorrentista, contaCorrentista, tipoDocumento, documento);
		} else {
	...
	private TipoPessoa pedirTipoPessoa() {
		...
		do {
			...
			try {
				...
				System.out.println("  >>> Tipo pessoa inválido, " + "digite PF ou PJ.");
			}
	...
	private TipoDocumentoPF pedirTipoDocumento() {
		TipoDocumentoPF tipoDocumento = null;

		do {

			System.out.println("Digite o tipo de documento[CPF] ou " + "[RG]:");
			String documentoDigitado = scanner.nextLine();

			try {
				tipoDocumento = TipoDocumentoPF.valueOf(documentoDigitado.toUpperCase());
			} catch (IllegalArgumentException excecao) {
				System.out.println("  >>> Tipo documento inválido, " + "digite CPF ou RG.");
			}
	...
	private Integer pedirAgenciaConta() {
		...
		do {
			...
			System.out.println("Digite agência da conta docorrentista:");
